...
- **203 CPU-only tests passed** with the bilingual catalog contracts: API and probability contracts, bounded inputs, authentication, model identity, persistent transport, concurrent response matching, failure handling, strict text generation, capability discovery and calibration provenance.
...
- **33 JavaScript checks passed**: nine ticket parsing/export checks, fourteen ticket browser checks and ten playground browser checks with controlled HTTP fixtures. They cover English defaults, Chinese switching, frozen batch rules, auth, queue recovery, safe exports and selective old-demo-history clearing.
...
- **Bilingual catalog validation**: all 12 fictional examples ran in both languages on real CPU MiniLM and GPU DiffusionGemma, for 24 requests per backend. All returned the expected question IDs. The longest CPU candidate pair was 136 of 512 tokens. The GPU requests took 0.225–3.292 seconds end to end in this small sequential run, including transport. Outputs are retained locally; these checks establish runnable inputs, not task accuracy or English/Chinese equivalence. MiniLM misrouted the meeting owner in both languages and produced different feedback categories across languages, illustrating the small model’s limits.
- **Live gallery check**: all 12 bilingual cards, cross-language search, category filtering, 390px layouts and locale-aware loading worked without browser errors. Loading a preset sent no inference request; explicitly running the meeting example returned three real GPU results, and export preserved the exact request and response. Custom input survived browsing and switching language.
